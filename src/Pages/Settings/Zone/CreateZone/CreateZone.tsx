...
const CreateZone = () => {
  ...
  const handleGetUpazilaList = async () => {
  ...
  const handleGetUnionList = async () => {
  ...
  //useLazyGetIsEmployeeAssignedInZoneQuery end==========================

  const [createZone, { isLoading: isLoadingCreateZone }] =
    useCreateZoneMutation();
        ...
        <form onSubmit={handleSubmit}>
          ...
          <section className="flex justify-end items-center mt-5">
            <CButton
              variant="outline"
              type="submit"
              disabled={isErrorIsEmployeeAssignedInZone || isLoadingCreateZone}
              loading={isLoadingCreateZone}
            >
              Submit
            </CButton>
          </section>
